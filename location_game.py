...
class LocationGame(Location):
  def __init__(self, g):
    ...
    self.buttons = [
      Button((self.win_w - 60, 10, 50, 50), menu_btn_img, self.font, "| |", (0,0,0,150), self.shower_location("menu"), "current_settings['control'] == 'touch'"),
      Button((20, self.win_h - 70, 200, 50), btn_img, self.font, ru("{player.weapon[name]}"), (0,0,0,150), self.player.change_weapon,
        "current_settings['control'] == 'touch'"),
      Button((240, self.win_h - 70, 200, 50), btn_img, self.font, ru("перезарядка"), (0,0,0,150), self.player.reload_weapon,
        "current_settings['control'] == 'touch' and player.weapon['type'] != 2")
    ]

    self.labels = [
      Label((20, 110), self.font, ru("{player.weapon[name]}"), (0,0,0,150)),
      Label((20, 150), self.font, "clip: {player.weapon[clip]}/{player.weapon[full clip]}", (0,0,0,150), (255,255,255), "player.weapon['type'] != 2"),
      Label((20, 190), self.font, "ammo: {player.weapon[ammo]}", (0,0,0,150), (255,255,255), "player.weapon['type'] != 2")
    ]
  ...
  def event(self, events):
    for event in events:
      if event.type == KEYDOWN and event.key == K_ESCAPE:
        return self.locations["menu"].show(self)
